...
export class PokemonSearch extends Component<User,SearchState> {
    ...
    onSearchClick = (): void => {
        ...
        let request = `https://pokeapi.co/api/v2/pokemon/${inputValue}`;
        fetch(request)
            .then((res) => {
                if(res.status!== 200){
    ...
    render() {
        ...
        if(error){
            ...
            resultMarkup = <div>
                ...
                <p>
                    {pokemon.name} has {pokemon.numberOfAbilities} abilities and {pokemon.baseExperience} base experince points 
                </p>
            </div>
        }
        return (
